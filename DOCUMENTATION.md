...
2. `events` - аргумент экзмепляра класса `EventEmitter`, который описывает работу брокера событий, например: добавление, подписку, удаления событий и т.д. В данном классе используется для вызова `super` классаом родителем. Имеет тип `IEvents`

3. `actions` - Используется для передачи события по клику кнопки "Далее" после заполнения полей контактов, имеет тип `TOrderActionss`.

В классе используeтся сеттер:
`address` - принимает аргументом строку, реализует установку значения поля.
...
В классе используeтся сеттеры:
`phone` - принимает аргументом строку, реализует установку значения поля.
`email` - принимает аргументом строку, реализует установку значения поля.
////////////`Contacts`/
